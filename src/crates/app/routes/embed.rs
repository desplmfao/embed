...
pub async fn embed(
    ...
    let q_url = urlencoding::decode(&q_urlr.to_string()).unwrap().to_string();

    if request.uri().path_and_query().unwrap().to_string().contains(&"/-?url=") {
        let data = serde_json::json!({
            "url": ("/-.mp4?url=".to_owned() + &q_url),
            "media_url": urlencoding::decode(&q_url).unwrap()
        });
        ...
        if q_url.starts_with("https://cdn.discordapp.com/attachments/") {
